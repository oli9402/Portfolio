...
In `for i in li:` the keyword `for` creates an iterator and passes the provided list. The iterator points to the first element of the list and the object is returned. The we call the `__next__()` method of this object to get the second object in li until `__next__()` raises an *StopIteration* exception
